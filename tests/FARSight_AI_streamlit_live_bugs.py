...
def analyze_text(text, source_name, model_name, user_addition=None):
    base_prompt = (
    ...
    if user_addition:
        base_prompt += "\n\nAdditional user guidance: " + user_addition.strip()
    base_prompt += "\n\n" + text.strip()
...
if uploaded_excel:
    ...
    if "prev_company_col" not in st.session_state:
        st.session_state.prev_company_col = None
    if "prev_website_col" not in st.session_state:
        st.session_state.prev_website_col = None

    if (
            st.session_state.prev_company_col != company_col
            or st.session_state.prev_website_col != website_col
    ):
        # Clear per-company session state to force reanalysis
        to_clear = [k for k in st.session_state.keys() if k.startswith("company_")]
        for k in to_clear:
            del st.session_state[k]
        # Also clear analysis_results
        st.session_state.analysis_results = {}
        st.session_state.prev_company_col = company_col
        st.session_state.prev_website_col = website_col
    ...
    if "prev_user_prompt_addition" not in st.session_state:
        st.session_state.prev_user_prompt_addition = ""

    user_prompt_addition = st.text_area(
        "🔤 Add custom guidance for functional analysis (optional)",
        placeholder="E.g., Focus on research and development functions, or compare product segments, etc."
    )

    # Clear analysis cache on user prompt change
    if st.session_state.prev_user_prompt_addition != user_prompt_addition:
        keys_to_clear = [k for k in st.session_state.keys() if k.startswith("company_")]
        for k in keys_to_clear:
            del st.session_state[k]
        st.session_state.analysis_results = {}
        st.session_state.prev_user_prompt_addition = user_prompt_addition
        ...
        with col1:
            if "website_analysis" not in st.session_state[company_key]:
                ...
                if website_text:
                    ...
                    website_analysis = analyze_text(website_text, f"{company_name} Website", gpt_model, user_prompt_addition)
                else:
            ...
            if pdf_file is not None:
                if st.session_state[company_key].get("last_pdf_filename") != pdf_file.name:
                    ...
                    if pdf_text:
                        ...
                        pdf_analysis = analyze_text(pdf_text, f"{company_name} Annual Report", gpt_model, user_prompt_addition)
                    else:
